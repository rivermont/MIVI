...
Observations needing phenology annotation: 8,624
...
```{R}
latitude_models <- function(df, nt) {
    # Create groups
    df$group <- ntile(df$latitude, n)
    
    # mean lat for each group
    a <- df %>% group_by(group) %>% summarise(mlat=mean(latitude, na.rm=TRUE), minlat=min(latitude, na.rm=TRUE), maxlat=max(latitude, na.rm=TRUE))
    
    # first seeding date in each group
    b <- df %>% group_by(group) %>% group_map(~first_seed(.x))
...
latitude_models(mivi_annotated, n)
```
