...
a <- mivi_annotated %>% group_by(group) %>% summarise(medlat=mean(latitude, na.rm=TRUE), minlat=min(latitude, na.rm=TRUE), maxlat=max(latitude, na.rm=TRUE))

# first fruiting date in each group
b <- mivi_annotated %>% group_by(group) %>% group_map(~first_fruit(.x))

a <- bind_cols(a, do.call(rbind.data.frame, b)[,1]) %>% mutate(firstfruit = ...5) %>% select(-...5)
a <- remove_missing(a, finite=TRUE)  # remove any Inf's

# Plot with linear model
print(
    ggplot(a, aes(medlat, firstfruit)) + geom_point() + geom_smooth(method='lm') +
        ylab("Julian day") + xlab("mean latitude") + labs(title="First Fruiting Day by Latitude")
)

# Q-Q residual plot
model_lat <- lm(firstfruit~medlat, data=a)
res <- resid(model_lat)
qqnorm(res)
qqline(res)
...
cor.test(a$firstfruit, a$medlat, alternative="less")
# cor.test(a$firstfruit, a$medlat, alternative="less")$estimate
...
# mean lat for each group
a <- mivi_annotated %>% group_by(group) %>% summarise(medele=mean(elevation, na.rm=TRUE))

# first fruiting date in each group
b <- mivi_annotated %>% group_by(group) %>% group_map(~first_fruit(.x))

a <- bind_cols(a, do.call(rbind.data.frame, b)[,1]) %>% mutate(firstfruit = ...3) %>% select(-...3)
a <- remove_missing(a, finite=TRUE)  # remove any Inf's

# Plot with linear model
ggplot(a, aes(medele, firstfruit)) + geom_point() + geom_smooth(method='lm') +
    ylab("Julian day") + xlab("mean elevation") + labs(title="First Fruiting Day by Elevation")

# Q-Q residual plot
model_ele <- lm(firstfruit~medele, data=a)
res <- resid(model_ele)
qqnorm(res)
qqline(res)

# Pearson's correlation test
cor.test(a$firstfruit, a$medele, alternative="less")
